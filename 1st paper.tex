...
\begin{figure} 
  \centering
  \includegraphics[width=.6 \linewidth]{errcontour.eps}
  \caption{Contour plot of the relative error in $\ln(P(\Lambda>0|\gamma,N)$ obtained from calculating $P$ using the Gaussian approximation versus the exact numerical result. The trend for increasing $N$ is for the relative error to \emh{decrease}. \SH{someone add axes labels and correct axis values! code in dropbox (the legend is fractional error, we could change to \% if we want (i.e. by multiplying by 100)).}}
  \label{errcontour}
\end{figure}
...
\sh{In fact, in figure \ref{errcontour} we show a contour plot of the relative error in the calculated value of $\ln \left[P(\Lambda>0|N,\gamma)\right]$ obtained using the Gaussian approximation versus the exact numerical value (i.e. $(\ln P_{\rm Gauss})/(\ln P_{\rm exact}) -1$). The plot covers $\gamma \in (0.05,0.95)$ and $N \in (3,12)$, which is the range over which we can obtain the exact results. We see that nowhere in this range is the error in the Gaussian approximation more than $\simeq 1\%$ and the trend as $N$ increases is for this relative error to \emph{decrease} at all $\gamma$ values. This justifies the use of the Gaussian approximation at larger $N$. We choose to plot the relative error in $\ln P$, rather than $P$, because $P$ itself decreases exponentially with $N$ and we are ultimately interested in distinctions such as those between $P(\Lambda>0) \sim 10^{-500}$ and $\sim 10^{-490}$.}
...
\sh{In figure \ref{eigendist2} we show the information in a cumulative distribution plot. We also zoom in on the smallest eigenvalues in the right hand panel. We see that the eigenvalues are consistenly smaller for the marginal, $\Lambda=0$ minima.}
