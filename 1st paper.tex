...
This covariance matrix is far from diagonal; we facilitate our analysis by using a basis as close to diagonal as possible. The $\eta_i$ variables are already diagonal, as are the $\zeta_{ij}$ terms with $i\neq j$ but the $\zeta_{ii}$ variables are correlated to each other, and also  to $V$. We look for a set of $N$ linear combinations of these $N$ variables that is diagonal, choosing,
\begin{align}
...
This leaves us with three separate possible $x_n$-dependent integrals  to evaluate: ``denom'' (Eq. \ref{denom}), ``numer'' (Eq. \ref{numer}) and $p(\Lambda)$ (Eq. \ref{plam}).  The final quantity we want to evaluate is ``numer"/``denom".
