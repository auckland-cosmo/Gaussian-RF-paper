...
\begin{figure} 
  ...
  \caption{The expected distribution of eigenvalues at the peak value of $\Lambda$ for $N=100$. The horizontal axis corresponds to the eigenvalues at that point, while the vertical axis is the unnormalized expected number density of each eigenvalue\SH{we can normalise by simply dividing by $N$} \lfl{Don't think that works, y-axis is $p(\lambda)$ so the area under the curve needs to be 1}. Note the distribution is independent of $\gamma$ (see text). Compare Figures 8 and 9 in Yamada and Vilenkin (\cite{Yamada2018}).\SH{change x-axis label to $\lambda/\sigma_2$}}
  \label{eigendist}
\end{figure}
