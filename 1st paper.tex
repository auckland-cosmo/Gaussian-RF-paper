...
\end{equation} 
%
as an unnormalised probability density that gives the (relative) likelihood of finding a minimum at which the landscape has a $V$, and the Hessian matrix has eigenvalues $\lambda_i$ as a function of the single independent variable $\gamma$. This expression is exact;  we immediately see that the prefactor to the exponential enforces a variant of {\em eigenvalue repulsion\/} \cite{Mehta1990} \re{cite RMT literature; in general you can have a symmetric matrix with degenerate eigenvalues}, in that the likelihood of finding a minimum with near-identical $\lambda_i$ is vanishingly small. 
