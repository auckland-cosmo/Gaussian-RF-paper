...
Our overall goal here is to understand the form $P(\Lambda)$ for  Gaussian random landscape potentials, the probability distribution that defines the distribution of vacuum energies at minima in the landscape. The overall probability Eq. \ref{DensityOfPeaks} depends on the value of $V$ and the moments of the power spectrum $\sigma_0, \sigma_1$,  and $\sigma_2$.  Since we will focus on a purely Gaussian landscape with no  physical parameters that set a scale we can work with both $\sigma_0$ and $\sigma_1$ equal to unity. The former is achieved by setting the average value of $V$ to zero by an additive rescaling, as can be seen from  Eq. \ref{corr}. Likewise, $\sigma_1$ responds to the average magnitude of the first derivatives and can be set to unity by rescaling the units of length, leaving $\sigma_2$ as the only nontrivial parameter. It can be shown that if $\sigma_0=\sigma_1=1$, $\sigma_2>1$; in much of what follows we  parameterise the landscape by $\gamma = 1/\sigma_2$ \re{should we actually show this; I know you sorted this out} and the dimensionality $N$,  in the knowledge that $0<\gamma<1$.

The probability $P$ gives the (relative) likelihood of finding a maximum \re{at what point do we start talking about minima, not maxima; we should probably flip it from the start} with value $V$ and the given values of $x_i$ which will, in turn, determine the eigenvalues of the Hessian at the minimum.  Up to an undetermined overall  constant the probability of obtaining a peak with value $V$ is given thus given by 
%
\begin{equation}
...
\begin{figure}
  ...
  \huge{FIGURE TO GO HERE}
%  \includegraphics[width=\linewidth]{N6minima.png}
  \caption{We show  realisations of a \re{(1 or 2D - you choose)} random function with $\gamma = 0.2$ and $\gamma=0.9$; the former case has a larger second moment and thus  more ``short scale'' power, allowing minima (maxima) to appear in significant numbers above (below) zero. Conversely with $\gamma$ close to unity the spectrum is dominated by longer wavelength modes, and most minima are low-lying. }
  \label{examples1}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{N6minima.png}
  \caption{The probability that a given minimum has $\phi > 0$ as a function of $\sigma_2$, for $N=5,6$. A close-up of the probabilities for small values of $\sigma_2$ is shown as well. The probability for $N=5$ is always larger than that for $N=6$. \re{Can we show these for $\gamma$ not $\sigma_2$ and make the plots in Mathematica, since they will be more easily tweaked there than in Excel}}
  \label{N6}
\end{figure}
...
\begin{figure}
  \centering
    \huge{FIGURE TO GO HERE}
%  \includegraphics[width=\linewidth]{N6minima.png}
  \caption{\re{A plot showing the value of $P_\pm$ for a few values of $\gamma$ as a function of $N$ with $N\le10$}}
  \label{gamma}
\end{figure}


For relatively small values of $N$ we can directly integrate over the $x_i$ to numerically evaluate $P(\Lambda)$ without any further approximations in a numerical algebra package; and for $N<10$ the VEGAS \cite{VEGAS} Monte Carlo integration routine in the GNU Scientific Library.\cite{GSL} gives results to several figures.   In order to characterise the landscape, consider 
%
...
\subsection{Gaussian Approximates to the Integral}

\re{put a write-up the Gaussian method here. I don't think we need the tables of values}
