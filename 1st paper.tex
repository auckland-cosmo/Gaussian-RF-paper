...
While stochastic (or eternal) inflation implies the existence of a multiverse composed of many pocket universes,  this does not require that the ``low energy" (i.e. LHC scale, in this context) physics or vacuum energy differs between pockets:  the naive quadratic inflation model supports eternal inflation, but has a unique vacuum.  In contrast, landscape models have multiple vacua which will, in principle, be populated by stochastic inflation or tunneling. The string landscape, built on the plethora of flux-stablilsed vacua that exist inside Calabi-Yau spaces, is well-known but it  is not necessarily the unique realisation of this scenario. The complexity of the landscape  and the vast number of vacua it supports is the basis of whatever explanatory power it possesses, and is almost uncountably large (e.g. $10^{500}$ or greater \cite{Douglas2003}), with the inference that essentially almost any value of the vacuum energy can be realised within it. 
...
This leaves us with three separate possible $x_n$-dependent integrals  to evaluate: ``denom'' (Eq. \ref{denom}), ``numer'' (Eq. \ref{numer}) and $p(\Lambda)$ (Eq. \ref{plam}). \LFL{The final quantity we want to evaluate is ``numer"/``denom".}

\LFL{Up to $N \sim 10$, we can evaluate both ``numer" and ``denom directly}. For $N>10$, direct calculation becomes very resource-intensive. To proceed further, we approximate each integrand as a Gaussian. We do this by fixing $\gamma$ and $N$, finding the location and value of the peak of the integrand and then numerically calculating the Hessian of the integrand with respect to the $x_n$ variables around this point. Thus we approximate each integral as
%
...
\begin{figure} 
  \centering
  \includegraphics[width=\linewidth]{arrayerrcontour.eps}
  \caption{Plot of the relative error in $\rm{log}(P(\Lambda>0|\gamma,N)$ obtained from calculating $P$ using the Gaussian approximation versus the exact numerical result. \LFL{Up to $N=8$, the trend for increasing $N$ is for the relative error to \emph{decrease}. Beyond this, and especially at small values of $\gamma$, we start seeing relatively large jumps in the error. Because we do not expect the errors to vary substantially for small changes in $N$ and $\gamma$, we attribute this to numerical noise.}}
  \label{errcontour}
\end{figure}
...
In Figure \ref{errcontour} we plot  the relative error in $\mathrm{log} \left[P(\Lambda>0|N,\gamma)\right]$ from the Gaussian approximation versus the exact numerical value (i.e. $(\rm{log} P_{\rm Gauss})/(\rm{log} P_{\rm exact}) -1$). The plot covers $\gamma \in (0.05,0.95)$ and $N \in (3,12)$; at larger $N$ the integrals become increasingly time consuming.  Within this range, the error in the Gaussian approximation never exceeds $\simeq 1\%$, and peaks at $N \simeq 9$. For $N\geq10$ the relative error \emph{decreases} for all $\gamma > 0.2$,  justifying the use of the Gaussian approximation at large $N$.\footnote{Moreover, the noisy results at large $N$ appear to be associated with the evaluation of the numerical integral.}A similar plot for $P$ would show a maximum error of $\sim 4\%$, and that this error increases with $N$. Extrapolating to $N\sim 150$ suggests that the Gaussian approximation would underestimate the normalised probability by a factor of a few \LFL{orders of magnitude} for the worst fitting value of $\gamma$. However, our analysis focuses on the logarithm of the probability; in what follows scenarios with (for example) $P(\Lambda>0) \sim 10^{-500}$ and $\sim 10^{-490}$ are functionally equivalent and the relative error in $\rm{log} P$ is thus the key quantity.  \re{reworded this, dropped reference to "contours" and Log should be log -- someone has starting writing in Mathematica. Wanted to be a little wary about that extrapolation so have "a few".} 
...
\lfl{In Figure \ref{eigendist}, we show the expected distribution, $\rho$, of the eigenvalues at minima with the most likely value of $\Lambda$ and for $\Lambda = 0$, for $N = 100, \gamma = 0.9$. Because the eigenvalue distribution at the most likely value of $\Lambda$ is independent of $\gamma$ (see previous paragraph), this result also holds for all $\gamma$. Our method provides us with the $x_i$'s (and by extension the eigenvalues) at these points. From here, we order the eigenvalues from smallest to largest as $\lambda_n$. If we invert this it gives us, for each value of $n$, the value of $\lambda$ for which there are $n$ eigenvalues equal or smaller, i.e. $n(\lambda)$. Other than at $\lambda\simeq 0$ this function is smooth. Therefore we can smoothly interpolate $n(\lambda)$ to non-integer values of $n$. We thus finally obtain $\rho(\lambda)$ by differentiating $n(\lambda)$ with respect to $\lambda$, i.e. $\rho(\lambda) = {\rm d}n/{\rm d}\lambda$. We expect this to be a good approximation of the true $\rho(\lambda)$ when the true $\rho(\lambda)$ has no sharp features, which is true everywhere except near $\lambda=0$.}
