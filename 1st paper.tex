...
\begin{figure} 
  ...
  \caption{We plot the minimum and maximum eigne}
...
\begin{figure} 
  ...
  \caption{We show the expected minimum and maximum eigenvalues at a minimum as a function of $N$ for four values of $\gamma$.  The dashed lines show the expected eigenvalues at the most likely value of $\Lambda$; the solid lines show the values at $V=0$. In all cases the expected values of the smallest eigenvalue decreases as $N$ increases, while the maximum value stays roughly unchanged. Moreover, if $\gamma$ is small  the difference between the  likely value of $\Lambda$ and $\Lambda=0$ is small, but this grows with $\gamma$.}
  \label{eigen}
\end{figure}
