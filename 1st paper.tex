...
\section{Comparison with Yamada and Vilenkin}
As mentioned in the Introduction, the paper by Yamada and Vilenkin \cite{Yamada2018} is closely related to ours. In particular, they have derived $p(\mathrm{min}|U, s.p.)$, which is related to our results by $p(F|\mathrm{min})= \frac{p(\mathrm{min}|F, s.p.)p(F|s.p.)}{p(\mathrm{min}|sp)}$\footnote{This result is derived using Bayes theorem, as well as the fact that minima are necessarily stationary points}.The numerator is independent of $F$, so since we are interested in the $p(F>0|\mathrm{min})=\int^\infty_0 p(F|\mathrm{min})dF/\int^\infty_{-\infty} p(F|min)dF$, it cancels. Therefore the two probabilities can be related wih knowledge of $p(F|s.p.)$. However, this quantity is not trivial to compute. We have been able to derive an analytical formula for this in 2D (see Appendix), but even in 2D the complexity is formidable.
...
\noindent where $\gamma = \sigma_1^2/\sigma_0\sigma_2$.
