...
\end{equation} 
%
$Q$ is given by Eq. \ref{Q}, $A$ is some constant factor, and the integration limits are only over the region where the conditions are satisfied. This is a key integral, and evaluating it will be the focus for much of the remainder of this paper.
...
We begin by making a qualitative exploration of the properties of the landscape. From a purely computational perspective, the $G$ term complicates what would otherwise be a relatively simple Gaussian function. However, note that the integrand vanishes when any two ``adjacent'' $\lambda_i$ become equal, enforcing an ``eigenvalue repulsion'' in the Hessian matrices for the maxima and minima.   

Here, we are interested in $P(F|min)$. Looking at Eq.~\ref{Q} we see that if $\gamma$ is close to unity, $Q$ becomes very large, which in turn exponentially suppresses the integral in Eq.~\ref{DensityOfPeaks}. Conversely, if $\gamma$ is very large, $\nu$ can take larger values for the same $Q$, i.e. we have more minima at large values of $\nu$. Figure~\ref{examples1} shows two illustrative examples; at larger values of $\gamma$ the power spectrum is effectively ``red'' and the overall function is dominated by longer wavelength modes. By constrast, when $\gamma$ is small the spectrum is ``blue'' and dominated by shorter wavelengths, making it more likely that both kinds of extrema can be found on either side of zero. Conversely, as $N$ increases $Q$ will always grow, making it less likely to find minima and maxima on the ``wrong'' side of zero. 

Consequently, we expect that for large $N$ (e.g. ${\cal{O}}(100)$) a Gaussian random landscape potential will have $P(F|min)$ such that $F < 0$ is strongly favoured over positive values. Beyond this, we need knowledge of the power spectrum and thus $\gamma$ to be able to draw detailed inferences about the form of $P(F|min)$. 
...
In order to characterise the landscape, consider 
%
\begin{equation} \label{Ppm}
P(F|min) = \frac{\int_0^\infty dF N_{peak}} {\int_{-\infty}^\infty dF N_{peak}} 
\end{equation}
%
which is a measure of the fraction of minima lying ``above the waterline'', or at positive values of the effective vacuum energy. For relatively small values of $N$ we can compute this integral without any further approximations in a numerical algebra package; and for $N\le10$ the VEGAS \cite{VEGAS} Monte Carlo integration routine in the GNU Scientific Library\cite{GSL} gives results to several figures. At larger $N$ we use the Monte Carlo integrator to evaluate $P(F|min)$ and a final trapezoidal integration over $\Lambda$  to evaluate $P_\pm$. 
